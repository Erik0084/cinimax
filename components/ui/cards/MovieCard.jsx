...
const MovieCard = ({ id, title, genre, imageUrl, series, rating, year }) => {
  ...
  const handlePress = () => {
    const href = series ? `/media/series/${id}` : `/media/movies/${id}`;
    router.push(href);
  };
      ...
      >
        <View className=" rounded-xl overflow-hidden relative">
          ...
          <View className="relative">
            ...
            {/* Play Button Overlay */}
            <View
              className="absolute inset-0 items-center justify-center opacity-0 hover:opacity-100 transition-opacity"
              pointerEvents="none"
            >
