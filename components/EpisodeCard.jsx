...
import { fetchEpisodes, fetchSeasons, JELLYFIN_URL } from '../utils/useJellyfin';
import SeasonSelectionModal from './SeasonSelectionModal';

const EpisodeCard = ({ episode, seriesId }) => {
            ...
            <View className="flex-row items-center mb-3">
                ...
                <View className="flex-1 flex-row  items-center py-2"> 
                    <View className="flex-1">
                        <Text className="text-grey text-h6">{formatDuration(episode?.RunTimeTicks)}</Text>
                        <Text className="text-white text-h4 font-bold">{episode?.Name || `Episode ${episode?.IndexNumber || 'N/A'}`}</Text>
                    </View>
...
const EpisodeList = ({ seriesId }) => {
    ...
    return (
        <View className="">
            ...
            >
                <Text className="text-white text-h4 font-semibold mr-2">{selectedSeason?.Name || 'Loading...'}</Text>
            ...
            {episodes.length === 0 && !loading && (
                <Text className="text-grey text-center py-4">No episodes available for this season.</Text>
            )}
